# Polynomials-For-Estimating-MRV

Polynomials for equating maximum recoverable volume from known effective set-rep schemes with other effective volume loads.

Take the ax^n-x=0 form of a binomial, and plug in a percentage of any rep max, such as a 10 rep max, and the number of sets you can do with that percentage as the exponent:

.85x^3-x=0

This should be something that currently doesn't plateau. Find its root. Any binomial of degree two or above sharing this root will have a coefficient (a) that corresponds to the weight that can be recovered from when sets are n.

In my experience, you can exceed MRV before reaching failure. This might help with that problem. 
